...
class DPSAdapter extends Controller
{
    ...
    public static function set_pxpost_account($pxPost_Username, $pxPost_Password)
    {
        self::$pxPost_Username = $pxPost_Username;
        self::$pxPost_Password = $pxPost_Password;
    }
    
    public static function set_pxpay_account($pxPay_Userid, $pxPay_Key)
    {
        self::$pxPay_Userid = $pxPay_Userid;
        self::$pxPay_Key = $pxPay_Key;
    }
    
    public static function get_pxpost_username()
    {
        return self::$pxPost_Username;
    }
    
    public static function get_pxpost_password()
    {
        return self::$pxPost_Password;
    }
    
    public static function get_pxpay_userid()
    {
        return self::$pxPay_Userid;
    }
    
    public static function get_pxpay_key()
    {
    ...
    public static function set_receipt_from($address)
    {
        self::$receipt_from = $address;
    }
    
    public static function get_receipt_from()
    {
    ...
    //Payment Function
    public function doPayment($inputs, $payment)
    {
        // Main Settings
        //$inputs = array();
        $inputs['PostUsername'] = self::$pxPost_Username;
        ...
        if ($responseFields['SUCCESS']) {
            ...
            if ($authcode = $responseFields['1']['AUTHCODE']) {
                $payment->AuthCode = $authcode;
            }
            if ($dpsBillingID = $responseFields['1']['DPSBILLINGID']) {
        ...
        if ($transactionRef = @$responseFields['DPSTXNREF']) {
            $payment->TxnRef = $transactionRef;
        }
        if ($helpText = @$responseFields['HELPTEXT']) {
            $payment->Message = $helpText;
        } elseif ($responseText = @$responseFields['RESPONSETEXT']) {
    ...
    public function doDPSHostedPayment($inputs, $payment)
    {
    ...
    {
        if (!$this->httpClient) {
            $this->httpClient = new DPSAdapter_HTTPClient();
        }
