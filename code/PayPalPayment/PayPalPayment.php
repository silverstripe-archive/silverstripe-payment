...
class PayPalPayment extends Payment
{
    ...
    public static function set_test_mode($test_account_email)
    {
        self::$test_mode = true;
    ...
    public static function set_account_email($account_email)
    {
    ...
    public static function set_continue_button_text($continue_button_text)
    {
    ...
    public static function set_header_image_url($header_image_url)
    {
    ...
    public static function set_header_back_color($header_back_color)
    {
        self::$header_back_color = $header_back_color;
    }

    protected static $header_border_color;
    
    public static function set_header_border_color($header_border_color)
    {
    ...
    public static function set_payflow_color($payflow_color)
    {
        self::$payflow_color = $payflow_color;
    }

    protected static $back_color;
    
    public static function set_back_color_black()
    {
        self::$back_color = '1';
    }
    ...
    public static function set_image_url($image_url)
    {
    ...
    public static function set_page_style($page_style)
    {
    ...
    public function PayPalForm()
    {
        ...
        if (self:: $continue_button_text) {
            $inputs['cbt'] = self::$continue_button_text;
        }

        if (self::$header_image_url) {
            $inputs['cpp_header_image'] = urlencode(self::$header_image_url);
        }
        if (self::$header_back_color) {
            $inputs['cpp_headerback_color'] = self::$header_back_color;
        }
        if (self::$header_border_color) {
        ...
        if (self::$back_color) {
            $inputs['cs'] = self::$back_color;
        }
        if (self::$image_url) {
            $inputs['image_url'] = urlencode(self::$image_url);
        }
        if (self::$page_style) {
            $inputs['page_style'] = self::$page_style;
        }
        ...
        if ($member->hasMethod('getState')) {
            $inputs['state'] = $member->getState();
        }
        if ($member->hasMethod('getZip')) {
            $inputs['zip'] = $member->getZip();
        }
...
class PayPalPayment_Handler extends Controller
{
    ...
    public static $URLSegment = 'paypal';
