...
class WorldpayPayment extends Payment
{
    ...
    public static function set_test_mode_refused_transaction()
    {
        self::set_test_mode('REFUSED');
    }
    
    public static function set_test_mode_authorised_transaction()
    {
        self::set_test_mode('AUTHORISED');
    }
    
    public static function set_test_mode_error_transaction()
    {
        self::set_test_mode('ERROR');
    }
    
    public static function set_test_mode_captured_transaction()
    {
    ...
    public static function set_installation_id($installation_id)
    {
    ...
    public static function set_result_file($result_file)
    {
    ...
    public static function set_merchant_code($merchant_code)
    {
    ...
    public static function set_authorisation_mode_full()
    {
        self::$authorisation_mode = 'A';
    }
    
    public static function set_authorisation_mode_pre()
    {
        self::$authorisation_mode = 'E';
    }
    
    public static function set_authorisation_mode_post()
    {
    ...
    public static function set_authorisation_valid_from($authorisation_valid_from)
    {
    ...
    public static function set_authorisation_valid_to($authorisation_valid_to)
    {
        self::$authorisation_valid_to = $authorisation_valid_to;
    }
        
    public static $callback_password;
    public static function set_callback_password($pass)
    {
    ...
    public function WorldPayForm()
    {
        ...
        if (self::$result_file) {
            $inputs['resultFile'] = self::$result_file;
        }
        if (self::$merchant_code) {
            $inputs['accId'] = self::$merchant_code;
        } // Not sure about its exact syntax
        if (self::$authorisation_mode) {
            $inputs['authMode'] = self::$authorisation_mode;
        }
        if (self::$authorisation_valid_from) {
            $inputs['authValidFrom'] = self::$authorisation_valid_from;
        }
        if (self::$authorisation_valid_to) {
            $inputs['authValidTo'] = self::$authorisation_valid_to;
        }
        ...
        if ($member->hasMethod('getPostCode')) {
            $inputs['postcode'] = $member->getPostCode();
        }
        if ($member->hasMethod('getFax')) {
            $inputs['fax'] = $member->getFax();
        }
        ...
        foreach ($inputs as $name => $value) {
            $fields .= '<input type="hidden" name="' . $name . '" value="' . Convert::raw2xml($value) . '"/>';
        }
...
class WorldpayPayment_Handler extends Controller
{
    ...
    public static $URLSegment = 'worldpay';
